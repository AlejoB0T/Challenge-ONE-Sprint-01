...
botonDesencriptar.addEventListener('click',function(event){
    ...
    if (capturarTexto.value.length === 0 || capturarTexto.value.trim().length === 0 || validarEntrada(capturarTexto.value)){
        ...
        let textoDesencriptado = encriptarDesencriptarTexto(capturarTexto.value,reemplazo,vocales); //invierto los parametros para que decodifique
        //Puede parecer repetitivo y poco eficiente el código, pero de esta manera si alguién tiene un texto ya encriptado podrá desencriptarlo desde el principio.
